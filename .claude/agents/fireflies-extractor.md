...
1. **When transcript corruption obscures an item, cross-reference the summary.** Verified 2026-05-01: the summary sometimes disambiguates speaker/owner attribution that the transcript mangled. If the summary's version of the item is cleaner (e.g., it cleanly attributes a quote that the transcript scrambled), use the summary's attribution + your best-effort verbatim from the cleanest portion of the transcript line. Set `confidence: medium` and add tag `summary-disambiguated` so a human can check.
